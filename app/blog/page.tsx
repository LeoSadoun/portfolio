  ...
  return (
    <main className="min-h-screen bg-nord-0 pt-24 pb-20 px-6">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <Link href="/" className="text-xs text-nord-4 hover:text-nord-8 transition-colors mb-6 inline-block">
            ← back to portfolio
          </Link>
          <p className="text-nord-14 text-xs mb-2 font-mono">// LAB_WRITEUPS</p>
        ...
        {posts.length === 0 ? (
          <div className="bg-nord-1/40 border border-nord-3/20 rounded-lg p-8 text-center">
            <p className="text-nord-4 text-sm font-mono">
              <span className="text-nord-14">$</span> ls content/blog/
            </p>
            <p className="text-nord-4 text-xs mt-2 opacity-60">
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {post.description && (
                      <p className="text-nord-4 text-xs mt-1 leading-relaxed">{post.description}</p>
                    )}
                    {post.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {post.tags.map((tag) => (
                          <span
                            key={tag}
                            className="text-[10px] px-1.5 py-0.5 border border-nord-3/20 text-nord-4 rounded"
                          >
                            {tag}
                          </span>
                        ))}
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-nord-4 text-xs">{post.date}</p>
                    {post.readingTime && (
